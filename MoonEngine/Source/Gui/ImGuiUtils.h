...
namespace MoonEngine
{
	struct ImGuiUtils
	{
		...
		static void AddPaddingToRect(float offset)
		{
			ImVec2 p0 = ImGui::GetCursorScreenPos();
		...
		static void SeparatorDistanced(float up, float down)
		{
			AddPadding(0.0f, up);
			ImGui::Separator();
			AddPadding(0.0f, down);
		}
		...
		{
			auto& io = ImGui::GetIO();
			ImFont* font = io.Fonts->AddFontFromFileTTF(path, pixelSize, config, io.Fonts->GetGlyphRangesJapanese());
			if (setDefault)
				io.FontDefault = font;
			return font;
		}

		static void AddIconFont(const char* path, float pixelSize, ImWchar minRange, ImWchar maxRange)
		{
			auto& io = ImGui::GetIO();
			...
			config.GlyphExtraSpacing.x = pixelSize * 0.2f;
			config.OversampleH = 3;
			config.OversampleV = 1;

			config.GlyphOffset.y = pixelSize * 0.2f;

			const static ImWchar icon_ranges[] = { minRange, maxRange, 0 };
			io.Fonts->AddFontFromFileTTF(path, pixelSize, &config, icon_ranges);
		}

		static void StyleCustomDark(bool cartoony = false)
		{
		...
		void StyleCustomGray(bool is3D = true)
		{
			ImGuiStyle& style = ImGui::GetStyle();
			ImVec4* colors = style.Colors;
